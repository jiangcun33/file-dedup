...
- ✅ **批量快速选择**：全选 / 反选 / 取消；按文件名长度、路径名长度、路径深度、创建/修改时间、文件名关键词、指定目录内/外 勾选（应用即清除旧勾选，无匹配的组整组保留）
- ✅ **批量设置保留**：按条件设置保留（文件名/路径名长短、路径深浅、时间新旧、指定目录内/外）；符合条件文件保留、其余勾选，无符合条件的组整组保留
...
- ✅ **Windows 11 Fluent 界面**：系统强调色、深浅模式自动适配、8px 圆角、Surface 卡片、自定义标题栏、下划线 Tab、Win11 风格控件（开关/复选框/输入框）、状态栏、文件右键菜单、150-200ms 过渡动画
- ✅ **结果表格化**：统一表格展示所有重复文件（组列合并 + 组间横线 + 「组 N」全选切换按钮），列宽全局拖动同步，选中文件显示红色「删除」标签
